...
class task_writer():
    ...
    def set_file(self):
        ...
        fileName = data_dir + '/RDM_PD' + 'participantX' + '_today' + '.csv'
        cnt = 0

        #Make sure we do not overwrite our data
        while os.path.exists(fileName):
            cnt += 1
            fileName = data_dir + '/RDM_PD' + 'participantX' + '_today' + '_v' + str(cnt) + '.csv'

        # Get the Fieldnames
        fnames = list(self.trial_dict.keys()) 
        # write a file
        self.dataFile = open(fileName, 'w')  # a simple text file with 'comma-separated-values'
        self.writer = csv.DictWriter(self.dataFile, fieldnames = fnames)
