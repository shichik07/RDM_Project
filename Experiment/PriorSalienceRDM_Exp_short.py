...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 23 18:05:48 2021

- Short version without mono condition
...
from prior_rdm.Task_func import item_struct_short as itm # generate Items
...
try: # if we have no participant number restart
    if int(inp.data[4])%2 == 0:
         print('We have a valid participant number')
except ValueError:
    print('Please indicate a participant number!')
    win.close()
    core.quit()
...
instruction_loop(INTRO_SHORT)

            
prtc_inst = PRACTICE_FULL_SHORT
task_inst = EXP_FULL_SHORT
Exp_info = 1 #remnant of the old task
for prac_idx, prac in enumerate(Practice):
       instruction_loop(prtc_inst[prac_idx]) #display the practice instructions
       Prtc_trials = lis.loc[(lis.Block == prac)] # Get practice trials
       if EEG_OPT == True:
           eeg_inter.practice_start_trigger()
       block_loop(Prtc_trials, Exp_info) #run practice
       if EEG_OPT == True:
           eeg_inter.practice_end_trigger()
Exp_trials = lis.loc[(lis.Block != Practice[0]) & (lis.Block != Practice[1])] # Get Task Trials
